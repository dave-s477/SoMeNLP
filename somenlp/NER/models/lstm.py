...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, device, char_size, word_size, tag2idx, emb_vecs, char_config, emb_config, drop_config, gen_config, **kwargs):
        ...
        for i in range(self.gen_config['layers']):
            input_size = ( self.emb_config['dim'] + (self.char_config['hidden_dim'] * 2) ) if i == 0 else self.gen_config['hidden_dim'] * 2
        ...
        self.crf = CRF(self.tag_size, self.device, init_parameters=None)
    ...
    def _get_lstm_features(self, sentence, char_feats, sequence_lengths):
        ...
        concat_feats = torch.cat((embeds, char_feats), dim=-1)
    ...
    def get_features(self, char_sentence, sentence, lengths):
        char_feats = self._get_character_features(char_sentence)
        word_feats = self._get_lstm_features(sentence, char_feats, lengths)
    ...
    def neg_log_likelihood(self, tags, **features):
        ...
        feats = self.get_features(features['char_sentence'], features['sentence'], features['lengths'])
